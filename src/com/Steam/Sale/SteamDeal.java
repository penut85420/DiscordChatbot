...
public class SteamDeal {
	...
	public void parse(){
		...
		while((i = s.indexOf("<dc:date>" + date)) != -1){
			...
			String title = contents.substring(contents.indexOf("<title><![CDATA[") + "<title><![CDATA[".length(), contents.indexOf("]]></title>"));
			contents = s.substring(i, s.indexOf("</item>"));
			String id = "";
			try {
				id = contents.substring(contents.indexOf("<steam:appid>") + "<steam:appid>".length(), contents.indexOf("</steam:appid>"));
			} catch (Exception e) {
			}
